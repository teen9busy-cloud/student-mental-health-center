...
-- PostgreSQL / Supabase Schema DDL (기존 simony1 프로젝트와 완벽 공존: smhc_ 접두사)
-- ==============================================================================

-- 1. 전담센터 정보 (진주 경상국립대병원 / 창원 경상국립대병원)
CREATE TABLE IF NOT EXISTS smhc_centers (
...
CREATE TABLE IF NOT EXISTS smhc_regions (
    id VARCHAR(20) PRIMARY KEY,
    name VARCHAR(50) NOT NULL,
    center_id VARCHAR(50) REFERENCES smhc_centers(id) ON DELETE SET NULL,
    office_of_education VARCHAR(100) NOT NULL,
    sort_order INT DEFAULT 0
);

-- 3. 시스템 사용자 (센터장/의사, 임상심리사, 사회복지사, 교육청 관리자)
CREATE TABLE IF NOT EXISTS smhc_users (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    auth_id UUID,
    center_id VARCHAR(50) REFERENCES smhc_centers(id),
    name VARCHAR(50) NOT NULL,
...
-- 4. 학생(내담자) 기본 정보
CREATE TABLE IF NOT EXISTS smhc_clients (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    client_code VARCHAR(30) UNIQUE NOT NULL, -- 가명 식별코드: STU-2026-001
    center_id VARCHAR(50) REFERENCES smhc_centers(id) NOT NULL,
    region_id VARCHAR(20) REFERENCES smhc_regions(id) NOT NULL,
    name VARCHAR(50) NOT NULL,
    ...
    risk_level VARCHAR(20) DEFAULT 'NORMAL' CHECK (risk_level IN ('NORMAL', 'MILD', 'MODERATE', 'SEVERE')),
...
CREATE TABLE IF NOT EXISTS smhc_psych_tests (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    client_id UUID REFERENCES smhc_clients(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS smhc_monitoring_logs (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    client_id UUID REFERENCES smhc_clients(id) ON DELETE CASCADE,
    client_name VARCHAR(50),
...
ALTER TABLE smhc_centers ENABLE ROW LEVEL SECURITY;
ALTER TABLE smhc_regions ENABLE ROW LEVEL SECURITY;
ALTER TABLE smhc_users ENABLE ROW LEVEL SECURITY;
ALTER TABLE smhc_clients ENABLE ROW LEVEL SECURITY;
ALTER TABLE smhc_psych_tests ENABLE ROW LEVEL SECURITY;
ALTER TABLE smhc_monitoring_logs ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Allow anon read smhc_centers" ON smhc_centers FOR SELECT USING (true);
CREATE POLICY "Allow anon all smhc_regions" ON smhc_regions FOR ALL USING (true);
CREATE POLICY "Allow anon all smhc_users" ON smhc_users FOR ALL USING (true);
CREATE POLICY "Allow anon all smhc_clients" ON smhc_clients FOR ALL USING (true);
CREATE POLICY "Allow anon all smhc_psych_tests" ON smhc_psych_tests FOR ALL USING (true);
CREATE POLICY "Allow anon all smhc_monitoring_logs" ON smhc_monitoring_logs FOR ALL USING (true);

-- 초기 기본 전담센터 데이터 삽입
INSERT INTO smhc_centers (id, name, hospital_name, region_scope, address, phone) VALUES
...
INSERT INTO smhc_regions (id, name, center_id, office_of_education, sort_order) VALUES
